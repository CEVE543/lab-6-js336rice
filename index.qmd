...
## Part 5: Make our own plots 

### Select a rectangle 

Just for fun, let's select the Great Lakes Region of North America 
```{julia}
data_rectangle = ct1_slice.data[265:290,40:50];


fig, ax, plt = heatmap(data_rectangle; colormap = :seaborn_icefire_gradient,
    axis = (; aspect=DataAspect()),
    figure = (; size = (1200,600), fontsize=24))
fig
```

### Compute average TAS over our region 

Get TAS in our region for all time: 
```{julia}

#dims(c)
#Get the TAS across all time for our region:
#tas_region_over_time = c[265:290,40:50,:];
#use @view instead of the usual method to speed things up:
#tas_region_lazy_subset = @view c[265:290, 40:50, :]
...
#using YAXArrays
#using Statistics
#This is how the 'GEM' recommended I calculate the mean across the time series in a fast, efficient way.  The explanation it gave seemed "credible"

#tas_mean = mapreduce(identity, mean, tas_region_lazy_subset)

#calculate mean:
#tas_mean = mean(tas_region_lazy_subset)

```


### Fix for the data download issue: 

```{julia}
using Zarr, YAXArrays, Dates, NCDatasets, Statistics, DimensionalData
...
local_nc_path = "tas_region_GreatLakes.nc" # We will save our slice here as NetCDF but could use a different back end
...
if !isfile(local_nc_path)
    ...
    # the "gem" helped me with some fixes here... although at the time of writing this I think I'm still understanding the intricaciies of how the "do" function works, and why the original threw an error.
    #Follow-up on previous comment: I think I get it now.  I think it's as simple as the "do" block not being supported when the string is a url, and the "open_dataset" command had to happen on its own line as a result. 
    # 1. Open the remote Zarr store. zopen returns a ZGroup object.
    # We remove the 'do' block because zopen doesn't support it for a URL string input.
    z_group = zopen(remote_store_url, consolidated=true) 

    # 2. Convert the Zarr group 'z_group' into a YAXArray dataset 'g'
    g = open_dataset(z_group) 

    # Get the 'tas' variable (still lazy)
    tas = g["tas"]

    # Define the regional slice (still lazy)
    region_remote = tas[lon = 265 .. 290, lat = 40 .. 50]
    ...
    # No explicit close is needed for the remote Zarr store when just reading.
...
# Open the dataset (file handle may remain open until the variable is GC'd)
region = open_dataset(local_nc_path) 



#Select tas variable 
tas_array = region["tas"]



#Get the mean from each time step in a time series - this was semi-ai assisted, it gave me several wrong and weirdly complicated functions at first 
#mean_timeseries = mean(tas_array)

# 1. Define the dimensions you want to average over (the spatial dimensions)
spatial_dims = ("lat", "lon")
#spatial_dims = (:lat, :lon)

# 2. Apply the mean function over those dimensions
mean_time_series = mapslices(mean, tas_array, dims=spatial_dims)


```

```{julia}
# A way to display our results from the GEM: 
println("The mean time series is a YAXArray with dimensions:")
#It also gave the wrong function here.  I think it was pulling from standard julia methods of displaying dims, instead of yaxarray?  But I'm not sure. 
println(dims(mean_time_series))
println(dims(tas_array))
...
# Extract the data 
time_axis = collect(lookup(mean_time_series, :time))
#tas_values = collect(mean_time_series)
#Drop no longer used dimensions
tas_values = dropdims(collect(mean_time_series); dims=(1,2)) |> vec
# print(shape(time_axis))
# println(dims(tas_values))

# Create the figure and axis
f = Figure()
ax = Axis(f[1, 1], title="Time Series of Spatially Averaged TAS", xlabel="Time", ylabel="Mean TAS Value")

# Plot the line


lines!(ax, time_axis, tas_values, label="Mean TAS")

# Display the figure (if not in an environment that automatically displays)
display(f)

```

### Find the hottest and coldest time periods in our data 

I did this in a somewhat over-engineered way because I misread the instructions and was trying to find the 3 hottest and 3 coldest one-hour periods at first, and then retrofit that code to fit the actual assignment.  Still, this works and it's not many lines of code. 
```{julia}

#println(dims(mean_time_series))

#println(typeof(tas_values))

top_idx = partialsortperm(tas_values, 1; rev=true)  # indices of 3 largest values, the hottest

top_vals = tas_values[top_idx]
top_times = time_axis[top_idx]

println(top_vals)
println(top_times)

bottom_idx = partialsortperm(tas_values, 1; rev=false)  # indices of 3 smallest values, the coldest times 

bottom_vals = tas_values[bottom_idx]
bottom_times = time_axis[bottom_idx]

println(bottom_vals)
println(bottom_times)
```

### Plot the hottest and coldest time periods 
